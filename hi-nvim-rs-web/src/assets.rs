...
markup::define! {
    Github() {
        svg [class="icon", xmlns="http://www.w3.org/2000/svg", viewbox="0 0 98 96"] {
...
pub async fn favicon() -> impl IntoResponse {
    let svg = include_str!(concat!(env!("OUT_DIR"), "/favicon.svg"));
    (TypedHeader(ContentType::from(mime::IMAGE_SVG)), svg)
}
...
pub async fn css() -> impl IntoResponse {
    let css = concat!(
        ...
        "\n\n",
        include_str!(concat!(env!("OUT_DIR"), "/groups.css")),
    );
    (TypedHeader(ContentType::from(mime::TEXT_CSS)), css)
}
